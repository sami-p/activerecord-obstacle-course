...
# ----------------------
  ...
  it '12. should return all items except items 2, 5 and 6' do
    ...
    expected_result = [@item_1, @item_4, @item_9, @item_10, @item_3, @item_8, @item_7]
    ...
    # ------------------ Using ActiveRecord ----------------------
      items = Item.where.not(id: items_not_included.each { |item| item.id })
    # ------------------------------------------------------------
  ...
  it "13. groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
      ...
      grouped_items = order.items.select('items.*').group(:id, :name)
    # ------------------------------------------------------------
